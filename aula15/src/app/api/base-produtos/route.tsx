...
export async function POST(request:Request) {
    ...
    //Recebendo o dados da nossa PSEUDO-BASE de dados o arquivo .json
    //OBS: Lembre que ao recuperar os dados do arquivo, ele está em modo de string/json e 
    // não em objeto, por isso precisamos fazer o parse para transformar em objeto 
    const file = await fs.readFile(process.cwd() + '/src/data/base.json','utf-8');
    // Realizando o parse de string para objeto literal
    const produtos: TipoProduto[] = JSON.parse(file)

    // Aqyui estamos recebendo o request do cliente:
    //OBS: O cliente pode ser um testador de API que gere requisições do tipo POST, ou um formulário que envia dados para a API.
    ...
    //Devemos pegar o produto e guardar na lista de produtos
    produtos.push(produto);

    // Agora que temos o produto inserido na lista, devemos salvar a lista no arquivo.json
    //OBS: aqui estamos usando o método writeFileSync, que é um método síncrono
    // ou seja, ele não vai continuar a execução do código enquanto ele não terminar de escrever no arquivo

    // Não podemos esquecer de realizar o parse da lista de produtos para STRING
